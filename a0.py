...
def all_within_grid(coords: list[tuple[int, int]], n: int) -> bool:
    ...
    >>> all_within_grid([(0,0),(1,1),(2,2),(3,3)], 4)
    True
    >>> all_within_grid([(8,7),(1,1),(2,2),(3,3)], 4)
    False
    """
    for coord in coords:
...
def reflect_vertically(coord: tuple[int, int], n: int) -> tuple[int, int]:
    ...
    >>> reflect_vertically((1,1), 5)
    (3, 1)
    >>> reflect_vertically((3,4), 6)
    (2, 4)
    """
    num_x = coord[0]
    num_y = coord[1]
...
def reflect_points(line: list[tuple[int, int]],
    ...
    >>> reflect_points([(0,1),(1,2),(2,3)], 4)
    [(3, 1), (2, 2), (1, 3)]
    >>> reflect_points([(2,2), (2,3)], 6)
    [(3, 2), (3, 3)]
    """
    new_line = []
    for coord in line:
        new_line.append(reflect_vertically(coord, n))
    return new_line
...
def _is_diagonal(squares: list[Square], up: bool) -> bool:
    ...
    - len(squares) > 3
    """
    cur_row = squares[0].coord[0]
    cur_col = squares[0].coord[1]
    for square in squares[1:]:
        if up is True:
            if square.coord[0] - cur_row != -1 or \
                    square.coord[1] - cur_col != 1:
                return False
        else:
            if square.coord[0] - cur_row != 1 or \
                    square.coord[1] - cur_col != 1:
...
class Line:
    ...
    Representation Invariants:
        - len(self.cells) > 3
        - is_row(self.cells) or is_column(self.cells) or is_diagonal(self.cells)
    ...
    def is_full(self) -> bool:
        ...
        """
        square_filled = 0
        for square_ in self.cells:
            if square_.symbol is not None:
                square_filled += 1
        return square_filled == len(self.cells)
    ...
    def has_fiar(self, coord: tuple[int, int]) -> bool:
        ...
        >>> line.has_fiar((0, 2))
        True
        >>> line = Line([Square((0, 1)), Square((1, 1), 'X'),
        ...              Square((2, 1), 'X'), Square((3, 1), 'X'),
        ...              Square((4, 1), 'X')])
        >>> line.has_fiar((1, 1))
        True
        """
        coord_index = self._coord_to_location[coord]
        symbol = self.cells[coord_index].symbol
        if symbol is None:
            return False
        count = 1
        temp_coord_index = coord_index + 1
        while count < 4 and temp_coord_index < len(self.cells):
            other_square_right = self.cells[temp_coord_index]
            if other_square_right.symbol == symbol:
                count += 1
                temp_coord_index += 1
            else:
                break
        while count < 4 and coord_index >= 0:
            coord_index -= 1
            other_square_left = self.cells[coord_index]
            if other_square_left.symbol == symbol:
                count += 1
            else:
                break
        return count == 4
...
def create_rows_and_columns(squares: list[list[Square]]) -> \
    ...
    rows = []
    columns = []
    for coord_num in range(len(squares)):
        rows.append(Line(squares[coord_num]))
        column = []
        for row in squares:
            column.append(row[coord_num])
...
def create_mapping(squares: list[list[Square]]) -> \
    ...
    True
    """
    diagonal_lines = all_diagonals(squares)
    rows, columns = create_rows_and_columns(squares)
    mappings = {}
    ...
    for r in range(len(squares)):
        for c in range(len(squares)):
            lines_list = []
            coord = (r, c)
            lines_list.append(rows[r])
            lines_list.append(columns[c])
            mappings[coord] = lines_list

    for diagonal_line in diagonal_lines:
        for square_ in diagonal_line:
            mappings[square_.coord].append(diagonal_line)
    return mappings
...
def get_down_diagonal_starts(n: int) -> list[tuple[int, int]]:
    ...
    for i in range(n - 3):
        row_list.append((i, 0))
        column_list.append((0, i))
    start_coord_list = list(tuple(row_list[::-1]) + tuple(column_list))
    start_coord_list.remove((0, 0))
...
def get_down_diagonal(start: tuple[int, int], n: int) -> list[tuple[int, int]]:
    ...
    [(0, 0), (1, 1), (2, 2), (3, 3)]
    """
    down_diagonal_list = [start]
    for _ in range(n - 1):
        start = (start[0] + 1, start[1] + 1)
        if start[0] <= n - 1 and start[1] <= n - 1:
            down_diagonal_list.append(start)
    return down_diagonal_list
...
def get_all_down_diagonals(n: int) -> list[list[tuple[int, int]]]:
    ...
    """
    all_down_diagonals = []
    start_diagonals = get_down_diagonal_starts(n)
    for diagonal in start_diagonals:
        all_down_diagonals.append(get_down_diagonal(diagonal, n))
    return all_down_diagonals
...
def get_coords_of_diagonals(n: int) -> list[list[tuple[int, int]]]:
    ...
    for down_diagonal in down_diagonals:
        up_diagonals.append(reflect_points(down_diagonal, n))
        coords_of_diagonals = down_diagonals + up_diagonals
    return coords_of_diagonals
...
def all_diagonals(squares: list[list[Square]]) -> list[Line]:
    ...
    """
    length = len(squares)
    coord_of_all_diagonals = get_coords_of_diagonals(length)
    all_diagonal = []
    for diagonal in coord_of_all_diagonals:
        list_of_squares = []
        for coord in diagonal:
            list_of_squares.append(Square(coord))
        all_diagonal.append(Line(list_of_squares))
    return all_diagonal
...
class Grid:
    ...
    def drop(self, col: int, item: str) -> int | None:
        ...
        2
        """
        targeted_column = self._columns[col]
        return targeted_column.drop(item)
